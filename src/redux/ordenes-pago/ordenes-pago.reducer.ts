import { LambePaginator } from 'redux/lambe-paginator.model';
import * as ordenesPagoActions from 'redux/ordenes-pago/ordenes-pago.actions';

export interface OrdenesPagoState {
  paginator: LambePaginator;
  filtros: { banco: string; alias: string; numero_cuenta: string };
  loading: boolean;
  error: any;
  initialized: boolean;
  filterVisible: boolean;
  formVisible: boolean;
}

export const initState = {
  paginator: new LambePaginator(),
  filtros: { banco: null, alias: null, numero_cuenta: null },
  loading: false,
  error: null,
  initialized: false,
  filterVisible: false,
  formVisible: false,
};

export function OrdenesPagoReducer(
  state = initState,
  action: ordenesPagoActions.actions,
) {
  switch (action.type) {
    case ordenesPagoActions.LOAD_ORDENES_PAGOS:
      return {
        ...state,
        loading: true,
        initialized: true,
      };

    case ordenesPagoActions.LOAD_ORDENES_PAGOS_SUCCESS:
      return {
        ...state,
        paginator: {
          ...state.paginator,
          data: action.data,
          recordsFiltered: action.recordsFiltered,
        },
        loading: false,
        error: null,
      };

    case ordenesPagoActions.LOAD_ORDENES_PAGOS_FAIL:
      return {
        ...state,
        loading: false,
        error: action.error,
      };

    case ordenesPagoActions.CLEAR_ORDENES_PAGOS:
      return {
        ...state,
        paginator: {
          ...state.paginator,
          recordsFiltered: 0,
          data: [],
        },
        loading: false,
      };

    case ordenesPagoActions.CHANGE_ORDER:
      return {
        ...state,
        paginator: {
          ...state.paginator,
          parametros: {
            ...state.paginator.parametros,
            sort_field: action.field,
            sort_order: action.order,
          },
        },
        loading: true,
      };

    case ordenesPagoActions.CHANGE_PARAMS:
      return {
        ...state,
        paginator: {
          ...state.paginator,
          parametros: {
            ...state.paginator.parametros,
            page: action.params.page,
            page_size: action.params.page_size,
            sort_field: action.params.sort_field,
            sort_order: action.params.sort_order,
          },
        },
        loading: true,
      };
    case ordenesPagoActions.CHANGE_FILTER:
      return {
        ...state,
        filtros: { ...action.filter },
        loading: true,
      };

    case ordenesPagoActions.OPEN_FILTER:
      return {
        ...state,
        filterVisible: true,
      };
    case ordenesPagoActions.CLOSE_FILTER:
      return {
        ...state,
        filterVisible: false,
      };
    default:
      return state;
  }
}
